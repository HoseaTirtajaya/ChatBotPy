...
while x < 1: 
    ...
    for item in data["result"]:
        ...
        new_member = ""
        if "new_chat_participant" or "left_chat_participant" in item["message"]:
            if old_id < new_id:
                old_id = int(item["update_id"])
                try:
                    if "new_chat_member" in item["message"]:
                        welcome_msg(item, "welcome")
                    elif "left_chat_member" in item["message"]:
                        welcome_msg(item, "left")
                except:
                    pass 
